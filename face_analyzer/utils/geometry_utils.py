...
def relative_symmetry_score(mean_error: float, scale: float) -> float:
    """Convert mean mirror error to 0-100 symmetry score."""
    relative = mean_error / (scale + 1e-10)
    return float(np.clip(100.0 - relative * 400.0, 0.0, 100.0))
...
def compute_regional_asymmetry(
    ...
    symmetry_pct = float(np.clip(100.0 - relative * 100.0, 0.0, 100.0))
...
def cupid_bow_symmetry(
    ...
    return float(np.clip(100.0 - diff * 300.0, 0.0, 100.0))

...
class AdvancedGeometryAnalyzer:
    ...
    def _compute_eyes_advanced(
        ...
        metrics.eye_shape_symmetry = float(
            np.clip(100.0 - abs(left_shape - right_mirrored_ratio) / (left_shape + 1e-10) * 100, 0, 100)
        )
    ...
    def _compute_nose_advanced(
        ...
        if metrics.nostril_area_left > 0:
            metrics.nostril_symmetry = float(np.clip(
                100.0 - abs(metrics.nostril_area_left - metrics.nostril_area_right)
                / metrics.nostril_area_left * 100, 0, 100
            ))
    ...
    def _compute_jaw_advanced(
        ...
        metrics.jaw_definition_geometric = float(np.clip(
            (metrics.jaw_body_length / (jaw_h + 1e-10)) * 15, 0, 100
        ))

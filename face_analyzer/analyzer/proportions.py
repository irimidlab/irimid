...
class ProportionAnalyzer:
    ...
    def analyze(self, landmarks: FaceLandmarks) -> ProportionMetrics:
        ...
        brow_y = float(np.mean([
            points[i][1] for brow in (LEFT_BROW, RIGHT_BROW) for i in brow
            if i < len(points)
        ]))
        subnasale_y = points[SUBNASALE][1]
        chin_y = points[CHIN][1]

        face_height = chin_y - forehead_y
        upper_third = (brow_y - forehead_y) / face_height if face_height > 0 else 0
        middle_third = (subnasale_y - brow_y) / face_height if face_height > 0 else 0
        lower_third = (chin_y - subnasale_y) / face_height if face_height > 0 else 0

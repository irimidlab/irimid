...
class HarmonyAnalyzer:
    ...
    def analyze(
        ...
        alignment_scores = [
            100.0 - min(nose.alignment_deviation * 60, 20),
            100.0 - min(lips.alignment_deviation * 60, 20),
            100.0 - min(chin.alignment_deviation * 60, 20),
            eyes.ocular_symmetry_index,
        ]
        alignment_component = sum(alignment_scores) / len(alignment_scores)

        third_dev = (
            abs(proportions.upper_third_ratio - 0.33)
            + abs(proportions.middle_third_ratio - 0.34)
            + abs(proportions.lower_third_ratio - 0.33)
        )
        distribution_component = float(np.clip(100.0 - third_dev * 80, 65.0, 100.0))
